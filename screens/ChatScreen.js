...
class ChatScreen extends React.Component {
  ...
  async componentDidMount() {
    var blockedId = await AsyncStorage.getItem("BlockLists");
    let BlockedUser = JSON.parse(blockedId);
    if (BlockedUser && BlockedUser.length > 0) {
      this.setState({ blockedId: BlockedUser })
    }
  ...
  _createChatLink(chatId, chatInfo) {
    if (this.state.blockedId.length != 0) {
      let user1 = chatInfo.users[0];
      let user2 = chatInfo.users[1];
      this.state.blockedId.map((Bl, i) => {
        if (user1 == Bl.userID) {
